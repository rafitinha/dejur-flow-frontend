...
export const cobrancaTitulosSchema = z
  .object({
    checklistType: z.literal('COBRANCA_TITULOS'),
    ...
    title: z.object({
      ...
      protestCertificate: z.boolean(),
    }),
    otherGuarantees: z.boolean(),
    otherGuaranteesDetails: z.string().nullable().optional(),
    ...
    opinion: z.object({
      collectionCompanyConclusion: z.enum([
        'FAVORAVEL_GEQ',
        'DESFAVORAVEL_GEQ',
        'NAO_HOUVE',
      ]),
      ...
      policyObserved: z.boolean(),
      position: z.enum(['FAVORAVEL_AJUIZAMENTO', 'CONTRA_AJUIZAMENTO']),
    }),
  ...
  .superRefine((data, ctx) => {
    if (data.title.type === 'DUPLICATA') {
      ...
      if (
        !types.includes('NOTA_FISCAL') ||
        !types.includes('COMPROVANTE_ENTREGA')
      )
        ctx.addIssue({
          code: 'custom',
          message: 'Duplicata exige Nota Fiscal e comprovante de entrega.',
        });
    }
  });
